...
const NewVisitor: React.FC<INewVisitorProps> = (props) => {
  ...
  const saveVisitor = async () => {
    ...
    try {
      ...
      const locationCode = bldg ? bldg.LocationCode : '';
  ...
  const handleDeleteVisitor = (i: number) => { const l = [...visitorDetailsList]; l.splice(i, 1); setVisitorDetailsList(l); if (l.length === 0) setErrors(prev => ({ ...prev, Details: 'Visitor Details are required.' })); };
  ...
  const handleDialogClose = (confirmed: boolean) => { setDialogOpen(false); if (!confirmed) return; if (dialogMessage.includes("save") || dialogMessage.includes("submit")) saveVisitor(); else window.open(props.siteUrl, "_self"); };
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        setIsLoading(false);
      } catch (error) { console.error("Init error:", error); setIsLoading(false); }
    };
    init();
  }, []);

  if (isLoading) return <Backdrop className={classes.backdrop} open={true}><CircularProgress color="inherit" /></Backdrop>;

  if (showPrivacyModal && !privacyConsentGiven) return <PrivacyModal onAccept={handlePrivacyAccept} onDecline={handlePrivacyDecline} context={props.context} refNo={refNo} />;
  ...
  return (
    <form noValidate autoComplete="off">
      <div className={classes.root} style={{ padding: '12px' }}>
        ...
        <Backdrop className={classes.backdrop} open={isProgress}><CircularProgress color="inherit" /></Backdrop>
        <Snackbar open={isSavingDone} autoHideDuration={2000}>
          <Alert severity="success">
            Data has been saved successfully.
            {submitType === 2 && refNo && <div style={{ marginTop: '5px' }}><strong>Reference Number: {refNo}</strong></div>}
            {(isEncoder || isReceptionist) && submitType === 2 && <div style={{ marginTop: '5px' }}>An email notification has been sent to {approverDetails.name}.</div>}
          </Alert>
        </Snackbar>
      </div>
    </form>
