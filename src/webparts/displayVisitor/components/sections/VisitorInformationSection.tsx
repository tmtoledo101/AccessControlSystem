...
const VisitorInformationSection: React.FC<IVisitorInformationSectionProps> = (props) => {
  const { visitor, errorFields, isEdit, isEncoder, isReceptionist, isApproverUser, isSSDUser, purposeList, deptList, bldgList, contactList, isAC1Open, onChangeTxt, onChangeCbo, onDateTimeVisitChange, onACSelectedValue, onFindUser, onACOpen, onACClose, onChangeDropZone, onChipClick } = props;
  ...
  return (
    <>
        ...
        <Paper variant="outlined" className={classes.paper}>
          {checkVisibility('cedit') && (
            <FormControl className={classes.textField} error={!!errorFields.Purpose}>
              ...
              <Select labelId="purposeLabel" id="Purpose" value={visitor.Purpose} onChange={onChangeCbo} name='Purpose'>
                {purposeList.map((item) => (<MenuItem key={item.Title} value={item.Title}>{item.Title}</MenuItem>))}
              </Select>
              <FormHelperText>{errorFields.Purpose}</FormHelperText>
            </FormControl>
          )}
          {checkVisibility('cedit') && visitor.Purpose === 'Others' && (
            <TextField inputProps={{ maxLength: 255 }} error={!!errorFields.PurposeOthers} required label="Others" name="PurposeOthers" onChange={onChangeTxt} value={visitor.PurposeOthers} variant="standard" className={classes.textField} helperText={errorFields.PurposeOthers} />
          )}
        ...
        <Paper variant="outlined" className={classes.paper}>
          {checkVisibility('deptedit') && (
            <FormControl className={classes.textField} error={!!errorFields.DeptId}>
              ...
              <Select labelId="deptLabel" id="Dept" value={visitor.DeptId} onChange={onChangeCbo} name='DeptId'>
                {deptList.map((item) => (<MenuItem key={item.Id} value={item.Id}>{item.Title}</MenuItem>))}
              </Select>
        ...
        <Paper variant="outlined" className={classes.paper}>
          {checkVisibility('cedit') && (
            <FormControl className={classes.textField} error={!!errorFields.Bldg}>
              ...
              <Select labelId="bldgLabel" id="bldg" value={visitor.Bldg} onChange={onChangeCbo} name='Bldg'>
                {bldgList.map((item) => (<MenuItem key={item.Title} value={item.Title}>{item.Title}</MenuItem>))}
              </Select>
      ...
      <Grid item xs={12} sm={6}>
        <Paper variant="outlined" className={classes.paper}>
          {checkVisibility('cedit') ? (
            <TextField inputProps={{ maxLength: 255 }} error={!!errorFields.RoomNo} required label="Room No." name="RoomNo" onChange={onChangeTxt} value={visitor.RoomNo} variant="standard" className={classes.textField} helperText={errorFields.RoomNo} />
        ...
        <Paper variant="outlined" className={classes.paper}>
          {checkVisibility('cedit') ? (
            <FormControl className={classes.textField} error={!!errorFields.EmpNo}>
              <Autocomplete freeSolo id="Contact" style={{ width: 300 }} open={isAC1Open} onChange={onACSelectedValue} onOpen={onACOpen} onClose={onACClose} getOptionSelected={(option, value) => option.EmpNo === value.EmpNo} getOptionLabel={(option) => option.Name} options={contactList} defaultValue={{ EmpNo: visitor.EmpNo, Name: visitor.ContactName }}
                renderInput={(params) => (<TextField {...params} onChange={onFindUser} label="Contact Person" variant="standard" helperText={errorFields.EmpNo} error={!!errorFields.EmpNo} />)} />
            </FormControl>
          ...
          {checkVisibility('cedit') ? (
            <FormControl className={classes.textField} error={!!errorFields.DateTimeVisit}>
              <MuiPickersUtilsProvider utils={DateFnsUtils}>
                <DateTimePicker error={!!errorFields.DateTimeVisit} disablePast format="MM/dd/yyyy HH:mm" label="Date and Time of Visit From" value={visitor.DateTimeVisit} onChange={(d) => onDateTimeVisitChange(d, 'DateTimeVisit')} InputProps={{ className: classes.dateField }} />
        ...
        <Paper variant="outlined" className={classes.paper}>
          {checkVisibility('cedit') ? (
            <FormControl className={classes.textField} error={!!errorFields.DateTimeArrival}>
              <MuiPickersUtilsProvider utils={DateFnsUtils}>
                <DateTimePicker error={!!errorFields.DateTimeArrival} disablePast format="MM/dd/yyyy HH:mm" label="Date and Time of Visit To" value={visitor.DateTimeArrival} onChange={(d) => onDateTimeVisitChange(d, 'DateTimeArrival')} InputProps={{ className: classes.dateField }} />
              </MuiPickersUtilsProvider>
      ...
      {!(isApproverUser || isSSDUser) && (
        <Grid item xs={12} sm={12}>
          <Paper variant="outlined" className={classes.paper}>
            {checkVisibility('cedit') ? (
              <DropzoneArea acceptedFiles={['.docx', '.xlsx', '.xls', 'doc', '.mov', 'image/*', 'video/*', ' application/*']} showFileNames showPreviews maxFileSize={70000000} onChange={onChangeDropZone} filesLimit={10} showPreviewsInDropzone={false} useChipsForPreview previewGridProps={{ container: { spacing: 1, direction: 'row' } }} previewChipProps={{ classes: { root: classes.previewChip } }} previewText="Selected files" dropzoneText="Add an attachment" initialFiles={visitor.initFiles} />
            ) : (
              <div className={classes.rootChip}>
                {visitor.initFiles.map((row) => (
                  <Chip key={row} icon={<AttachFileIcon />} label={row} onClick={(e) => onChipClick(e, row, 'inputFields')} variant="outlined" />
                ))}
              </div>
            )}
          </Paper>
      ...
      <Grid item xs={12}><Paper variant="outlined" className={classes.paper}><Box style={{ fontSize: "1rem" }}>Company Information</Box></Paper></Grid>

      <Grid item xs={12} sm={6}>
        <Paper variant="outlined" className={classes.paper}>
          {checkVisibility('cedit') ? (
            <TextField inputProps={{ maxLength: 255 }} error={!!errorFields.CompanyName} required label="Company Name" name="CompanyName" onChange={onChangeTxt} value={visitor.CompanyName} variant="standard" className={classes.textField} helperText={errorFields.CompanyName} />
      ...
      <Grid item xs={12} sm={6}>
        <Paper variant="outlined" className={classes.paper}>
          {checkVisibility('cedit') ? (
            <TextField multiline error={!!errorFields.Address} required label="Address" name="Address" onChange={onChangeTxt} value={visitor.Address} variant="standard" className={classes.textField} helperText={errorFields.Address} />
      ...
      <Grid item xs={12} sm={6}>
        <Paper variant="outlined" className={classes.paper}>
          {checkVisibility('cedit') ? (
            <TextField inputProps={{ maxLength: 255 }} error={!!errorFields.VisContactNo} required label="Contact No." name="VisContactNo" onChange={onChangeTxt} value={visitor.VisContactNo} variant="standard" className={classes.textField} helperText={errorFields.VisContactNo} />
      ...
      <Grid item xs={12} sm={6}>
        <Paper variant="outlined" className={classes.paper}>
          {checkVisibility('cedit') ? (
            <TextField inputProps={{ maxLength: 255 }} label="Local No." name="VisLocalNo" onChange={onChangeTxt} value={visitor.VisLocalNo} variant="standard" className={classes.textField} helperText={errorFields.VisLocalNo} />
      ...
      <Grid item xs={12} sm={6}>
        <Paper variant="outlined" className={classes.paper}>
          <div className={classes.datelabel}>
            <FormControlLabel control={<Checkbox checked={visitor.RequireParking} onChange={onChangeTxt} name="RequireParking" color="primary" disabled={!checkVisibility('cedit')} />} label="Request for Parking" />
          </div>
        </Paper>
      </Grid>

      <Grid item xs={12} sm={6}><Paper variant="outlined" className={classes.paper}><DisplayField label="Form Status" value={visitor.Status.Title} classes={classes} /></Paper></Grid>
    </>
  );
};
