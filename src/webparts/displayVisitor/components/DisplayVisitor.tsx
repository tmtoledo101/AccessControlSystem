...
export default function Approval(props: IDisplayVisitorProps) {
  ...
  const handleChipClick = (e, row, ctrl: string) => {
    ...
    link.click();
        ...
        const refNoMatch = emailProps.Subject.match(/([A-Z]{2,5}-\d{8}-\d{3})/);
        const referenceNo = refNoMatch ? refNoMatch[1] : '';
    
        console.log("Subject:", emailProps.Subject);
  ...
  const sendEmail = async () => {
    ...
    if ((isEncoder) && (sAction === 'submit') && (inputFields.StatusId === 1)) {
      ...
      const saved = await saveEmailData(emailProps, null);
      ...
      //await sp.utility.sendEmail(emailProps);
      await saveEmailData(emailProps, null);
